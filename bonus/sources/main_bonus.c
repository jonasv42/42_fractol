...
/*   Updated: 2023/12/15 20:29:51 by jvets            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
uint32_t	ft_color(int iterations, t_specs **specs)
{
	...
	(*specs)->green = 0;
	(*specs)->blue = 0;
	percentage = (double)iterations / (double)(*specs)->max_iterations;
	if (percentage < 0.33)
		(*specs)->red = 255 * (percentage / 0.33);
	else if (percentage >= 0.33 && percentage < 0.66)
		(*specs)->blue = 255 * (percentage / 0.66);
	else
		(*specs)->green = 255 * (percentage / 1);
	(*specs)->hex_value = ((*specs)->red << 24) | ((*specs)->green << 16)
		| ((*specs)->blue << 8) | 255;
